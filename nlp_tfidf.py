...
#     remove stopwords
    ...
    no_stopwords = no_digits.apply(lambda x : [word for word in words if word not in stop_words])
#     lemmatizing words
    lemmatizer = WordNetLemmatizer()
    lemmatized = no_stopwords.apply(lambda x: [lemmatizer.lemmatize(word) for word in x])
...
def create_description(lemmatized_names, lemmatized_specs):
    ...
    d = dd.apply( lambda x: x.item_name + x.item_feature, axis=1)
    descriptions = d.apply(lambda x: ' '.join(x))
...
if __name__= '__main__':
    names = pd.read_csv('names.csv',names=['index','item_name'], index_col='index')['item_name']
